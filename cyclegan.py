...
def get_transform(params=None, grayscale=False, method=Image.BICUBIC, convert=True):
    ...
    if params is None:
        ...
        transform_list.append(transforms.Lambda(
            lambda img: __crop(img, params['crop_pos'], 256)))
    if params is None:
        pass
...
generator = ResnetGenerator(input_nc=3, output_nc=3, ngf=64, norm_layer=norm_layer,
...
binary_np = np.fromstring(binary_arry, dtype=np.uint8)
...
A_img = cv2.imdecode(binary_np, cv2.IMREAD_COLOR)
